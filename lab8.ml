...
(* add_listener newswire {id = new_id (); action = fakeNewsNetwork} ;;
add_listener newswire {id = new_id (); action = buzzFake} ;; *)
...
let receive_report (s : string) : unit =
  ignore (add_listener publish (fun () -> print_string s)) ;;
...
let id3 = add_listener newswire receive_report ;;
